...
REAL(KIND=8) FUNCTION ints_mu0pi(nvib,a,b,mu0,phi2,zeta,&
  ...
  ELSE
    i = loc(0)
    j = loc(1) 
    s1 = s1 + zeta(i,j,b)*SQRT(phi2(j)/phi2(i))*&
      ...
      DO l=0,nvib-1
        DO k=0,nvib-1
          ...
          IF (ALL(bra(0:v(0)-1) .EQ. ket(0:v(0)-1)) .AND. &
            ...
            s1 = s1 + mu0(a,b)*zeta(i,j,a)*zeta(k,l,b)*&
                      SQRT(phi2(l)*phi2(j)/(phi2(k)*phi2(i)))*&
                      ints_eval_qpqp(nvib,[i,j,k,l],bra,ket)
          END IF
        END DO
      END DO
    END DO
      ...
      DO j=0,nvib-1
        DO i=0,nvib-1
          ...
          IF (ALL(bra(0:v(0)-1) .EQ. ket(0:v(0)-1)) .AND. &
              ...
              ALL(bra(v(2)+1:v(3)-1) .EQ. ket(v(2)+1:v(3)-1)) .AND. &
              ALL(bra(v(3)+1:nvib-1) .EQ. ket(v(3)+1:nvib-1)) ) THEN
            s1 = s1 + phi4(i,j,k,l)*ints_eval_qqqq(nvib,[i,j,k,l],bra,ket)
          END IF
        END DO
      END DO
    END DO
